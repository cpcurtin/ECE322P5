...
void eval(char *cmdline)
{
    ...
    char mod[MAXLINE];   // modified cmdline
...
int builtin_cmd(char **argv)
{
    ...
    else if ((strcmp(argv[0], "fg") == 0) || (strcmp(argv[0], "bg") == 0)) // bg or fg
    {
        do_bgfg(argv);
        return 1;
    }
...
void do_bgfg(char **argv)
{
    ...
    if (argv[1][0] == '%') // job condition
    {
        jobid = atoi(&argv[1][1]);
        temp_job = getjobjid(jobs, jobid);
        if (temp_job == NULL)
        {
    ...
    else if (isdigit(argv[1][0])) // process condition
    {
        pid = atoi(argv[1]);
    ...
    if (strcmp(argv[0], "fg") == 0) // hold fg process
    {
        temp_job->state = FG;
        waitfg(temp_job->pid);
    }
...
{
    pid_t pid;                                                                   // process id
    int child_status;                                                            // child status init
    while ((pid = waitpid(fgpid(jobs), &child_status, WNOHANG | WUNTRACED)) > 0) // reap child
    {
        struct job_t *temp_job = getjobpid(jobs, pid);
        if (WIFEXITED(child_status)) // normal delete
        {
            temp_job->state = UNDEF;
            deletejob(jobs, pid);
        }
        else if (WIFSIGNALED(child_status)) // print termination signal & delete
        {
            struct job_t *temp_job = getjobpid(jobs, pid);
        ...
        else if (WIFSTOPPED(child_status)) // status update stopped
        {
            struct job_t *temp_job = getjobpid(jobs, pid);
            temp_job->state = ST;
